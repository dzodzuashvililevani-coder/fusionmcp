...
def changed_line_numbers(before: str, after: str) -> list[int]:
...
def test_scalar_write_changes_one_line_and_preserves_comment(tmp_path):
    ...
    before_params = read(tmp_path, "params.yaml")
    before_measurements = read(tmp_path, "docs/measurements.md")
    field = field_by_id("stock_thickness")

    result = write_value(field, 2.7, root=tmp_path)

    after_params = read(tmp_path, "params.yaml")
    after_measurements = read(tmp_path, "docs/measurements.md")
    assert changed_line_numbers(before_params, after_params) == [11]
    assert before_params.count("#") == after_params.count("#")
    assert result.file == "params.yaml"
    assert result.line_number == 11
    assert result.old_text == "  thickness_mm: 3.0          # TODO measure with caliper\n"
    assert result.new_text == "  thickness_mm: 2.7          # TODO measure with caliper\n"
    assert result.checklist_ticked is True
    assert changed_line_numbers(before_measurements, after_measurements) == [45]
    assert "- [x] Actual thickness (measure, do not trust the label): 2.7 mm" in after_measurements


def test_inline_list_write_changes_only_target_section_element(tmp_path):
    copy_project_data(tmp_path)
    before = read(tmp_path, "params.yaml")
    field = field_by_id("battery_width")

    write_value(field, 34.5, root=tmp_path)

    after = read(tmp_path, "params.yaml")
...
def test_flow_map_write_changes_only_requested_token(tmp_path):
    copy_project_data(tmp_path)
    before = read(tmp_path, "components/loadout.yaml")
    field = field_by_id("esc_4in1_mass")

    result = write_value(field, 8.25, root=tmp_path)

    after = read(tmp_path, "components/loadout.yaml")
...
def test_preview_returns_diff_without_writing(tmp_path):
    ...
    assert "+  bolt_circle_mm: 9.4        # TODO measure hole-to-hole across the motor base\n" in diff
    assert "+- [x] Bolt circle (hole to hole, across the base): 9.4 mm\n" in diff
    assert read(tmp_path, "params.yaml") == before_params
    assert read(tmp_path, "docs/measurements.md") == before_measurements
...
def test_parse_failure_leaves_original_file_untouched(tmp_path, monkeypatch):
    ...
    assert read(tmp_path, "params.yaml") == before
...
def test_tick_measurement_handles_second_checkbox_on_same_line(tmp_path):
    copy_project_data(tmp_path)
    before = read(tmp_path, "docs/measurements.md")

    result = tick_measurement("Antenna mass and length", 3.2, "g", root=tmp_path)

    after = read(tmp_path, "docs/measurements.md")
    assert changed_line_numbers(before, after) == [42]
    assert result.line_number == 42
    assert "- [ ] Receiver mass: ____   - [x] Antenna mass and length: 3.2 g" in after
